...
def modal_transcribe(video_bytes: bytes, filename: str, model_name: str = "base") -> dict:
    """
    Call Modal transcribe_video function remotely.
    ...
    result = fn.remote(video_bytes=video_bytes, filename=filename, model_name=model_name)
    logger.info(f"Modal transcribe done: {result['merged_word_count']} words, lang={result['language']}")
    return result


def modal_burn_subtitles(video_bytes: bytes, subtitles: list, filename: str) -> bytes:
    """
    Call Modal burn_subtitles function remotely.
    ...
    result = fn.remote(video_bytes=video_bytes, subtitles=subtitles, filename=filename)
